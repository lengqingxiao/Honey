...
class CalculateTagsForHoneybee(ProcessDataForHoneybee):
    ...
    def calculate_call_list_func_t(self):
        ...
        if not self.contact_list.empty:
            ...
            for _i in range(len(cnt_list)):
                self.honeybee_info_result['cpc_total_%s_cnt' % cnt_result_list[_i]] = -999
            for _i in range(len(cnt_list[3:])):
                self.honeybee_info_result['cpc_total_%s_%s_cnt' % ('in', cnt_result_list[3:][_i])] = -999
                self.honeybee_info_result['cpc_total_%s_%s_cnt' % ('out', cnt_result_list[3:][_i])] = -999
            _time_tags = ['p1m', 'p1w', 'p3m', 'po3m']
            self.honeybee_info_result['cpc_total_call_pct'] = -999
            for _i in range(len(cnt_list[1:])):
                self.honeybee_info_result['cpc_total_%s_pct' % cnt_result_list[1:][_i]] = -999
            self.honeybee_info_result['cpc_call_in_only_pct'] = -999
            self.honeybee_info_result['cpc_call_out_only_pct'] = -999
            self.honeybee_info_result['cpc_call_both_pct'] = -999
            for _i in range(len(cnt_list[3:])):
                self.honeybee_info_result['cpc_total_%s_%s_pct' % ('in', cnt_result_list[3:][_i])] = -999
                self.honeybee_info_result['cpc_total_%s_%s_pct' % ('out', cnt_result_list[3:][_i])] = -999
            for _i in range(len(_time_tags)):
                for m in range(len(cnt_list[7:])):
                    self.honeybee_info_result['cpc_%s_pct_%s' % (cnt_result_list[7:][m], _time_tags[_i])] = -999
                    self.honeybee_info_result['cpc_%s_in_pct_%s' % (cnt_result_list[7:][m], _time_tags[_i])] = -999
                    self.honeybee_info_result['cpc_%s_out_pct_%s' % (cnt_result_list[7:][m], _time_tags[_i])] = -999
            self.honeybee_info_result['cpc_contact_all_day_pct'] = -999
            self.honeybee_info_result['cpc_call_lot'] = -999
            self.honeybee_info_result['cpc_call_in_lot'] = -999
            self.honeybee_info_result['cpc_call_out_lot'] = -999
            self.honeybee_info_result['cpc_contact_all_day_loc_cnt'] = -999
            self.honeybee_info_result['cpc_loc_cnt'] = -999

    def calculate_call_list_func_s(self):
        cnt_list = USER_CONTACT_COLUMNS
        cnt_result_list = USER_CONTACT_RESULT
        _time_tags = ['p1m', 'p1w', 'p3m', 'po3m']
        if not self.contact_list.empty:
            ...
            for _i in range(len(cnt_list[1:])):
                self.honeybee_info_result['cpc_%s_loc_cnt' % cnt_result_list[1:][_i]] = -999
            self.honeybee_info_result['cpc_call_in_only_loc_cnt'] = -999
            self.honeybee_info_result['cpc_call_out_only_loc_cnt'] = -999
            self.honeybee_info_result['cpc_call_both_loc_cnt'] = -999
            for _i in range(len(cnt_list[3:])):
                self.honeybee_info_result['cpc_total_%s_%s_loc_cnt' % ('in', cnt_result_list[3:][_i])] = -999
                self.honeybee_info_result['cpc_total_%s_%s_loc_cnt' % ('out', cnt_result_list[3:][_i])] = -999
            for _i in range(len(_time_tags)):
                for m in range(len(cnt_list[7:])):
                    self.honeybee_info_result['cpc_%s_loc_cnt_%s' % (cnt_result_list[7:][m], _time_tags[_i])] = -999
                    self.honeybee_info_result['cpc_%s_in_loc_cnt_%s' % (cnt_result_list[7:][m], _time_tags[_i])] = -999
                    self.honeybee_info_result['cpc_%s_out_loc_cnt_%s' % (cnt_result_list[7:][m], _time_tags[_i])] = -999
            cal_avg_list = ['cpc_%s_%s_%s' % ('%s', _i, '%s') for _i in cnt_result_list]
            for s in cal_avg_list:
                self.honeybee_info_result[s % ('avg', 'cnt')] = -999

    def calculate_call_list_func_r(self):
        # cnt_list = USER_CONTACT_COLUMNS
        cnt_result_list = USER_CONTACT_RESULT
        # _time_tags = ['p1m', 'p1w', 'p3m', 'po3m']
        if not self.contact_list.empty:
            ...
            self.honeybee_info_result['cpc_total_call_in_lot_ratio'] = -999
            self.honeybee_info_result['cpc_total_call_out_lot_ratio'] = -999
            cal_pct_ratio = ['cpc_total_%s_%s' % (_i, 'pct') for _i in cnt_result_list[1:]]
            cal_pct_ratio_r = ['cpc_%s_%s_ratio' % (_i, 'pct') for _i in cnt_result_list[1:]]
            for _i in range(len(cal_pct_ratio)):
                self.honeybee_info_result[cal_pct_ratio_r[_i]] = -999
            self.honeybee_info_result['cpc_call_in_only_ratio'] = -999
            self.honeybee_info_result['cpc_call_out_only_ratio'] = -999
            self.honeybee_info_result['cpc_call_both_ratio'] = -999
            cal_pct_ratio_c = ['cpc_%s_pct_%s_ratio' % (_i, '%s') for _i in cnt_result_list[3:]]
            _call_in = ['cpc_total_in_%s_%s' % (_i, '%s') for _i in cnt_result_list[3:]]
            # _call_out = ['cpc_total_out_%s_%s' % (_i, '%s') for _i in cnt_result_list[3:]]
            for _i in range(len(_call_in)):
                self.honeybee_info_result[cal_pct_ratio_c[_i] % 'in'] = -999
                self.honeybee_info_result[cal_pct_ratio_c[_i] % 'out'] = -999
                self.honeybee_info_result[cal_pct_ratio_c[_i] % 'total'] = -999
                # self.honeybee_info_result[cal_pct_ratio_c[_i] % 'total'] = -999
            cal_cnt_ratio = ['cpc_total_%s_%s' % (_i, 'cnt') for _i in cnt_result_list[1:]]
            cal_cnt_ratio_r = ['cpc_%s_%s_ratio' % (_i, 'cnt') for _i in cnt_result_list[1:]]
            for _i in range(len(cal_cnt_ratio)):
                self.honeybee_info_result[cal_cnt_ratio_r[_i]] = -999

    def calculate_call_list_info(self):

        self.calculate_call_list_func_t()
        self.calculate_call_list_func_s()
        self.calculate_call_list_func_r()
    ...
    def calculate_trip_info(self):
        ...
        if not self.trip_info.empty:
            ...
            _cal_pattern_result = {_k: _v for _k, _v in _cal_dict.items() if _v > 1}
    ...
    def calculate_region_info(self):
        ...
        self.calculate_region_info_func_t('cnt')
        self.calculate_region_info_func_t('time')
        if not self.contact_region.empty:
            ...
            self.honeybee_info_result.rename(columns=_unknown_dict, inplace=True)
            _r_result_tep_tuple = ('cpc_%s_region_loc_cnt', 'cpc_%s_region_loc_cnt_ratio', 'cpc_%s_pct',
                                   'cpc_%s_pct_ratio' )
            for _i in range(len(_cal_col_list)):
                for _r in _r_result_tep_tuple:
                    self.honeybee_info_result[_r % _cal_col_r_list[_i]] = -999
                # self.honeybee_info_result['cpc_%s_region_loc_cnt' % _cal_col_r_list[_i]] = -999
                # self.honeybee_info_result['cpc_%s_region_loc_cnt_ratio' % _cal_col_r_list[_i]] = -999
                # self.honeybee_info_result['cpc_%s_pct' % _cal_col_r_list[_i]] = -999
                # self.honeybee_info_result['cpc_%s_pct_ratio' % _cal_col_r_list[_i]] = -999
    ...
    def calculate_region_info_func_t(self, cal_tags):
        _cal_col_list = ['if_china', 'if_edp', 'if_ddc', 'if_dpt', 'if_risk']
        _cal_col_r_list = [_i[3:] for _i in _cal_col_list]
        _r_f = {'cnt': 'cpc_total_call_cnt', 'time': 'cpc_call_lot'}
        if not self.contact_region.empty:
            for _i in range(len(_cal_col_list)):
                _cal_data = self.contact_region[self.contact_region[_cal_col_list[_i]] == 1]
                self.honeybee_info_result['cpc_%s_call_in_%s' % (_cal_col_r_list[_i], cal_tags)] = \
                    _cal_data['region_call_in_%s' % cal_tags].sum()
                self.honeybee_info_result['cpc_%s_call_out_%s' % (_cal_col_r_list[_i], cal_tags)] = \
                    _cal_data['region_call_out_%s' % cal_tags].sum()
                self.honeybee_info_result['cpc_%s_call_%s' % (_cal_col_r_list[_i], cal_tags)] = \
                    self.honeybee_info_result['cpc_%s_call_in_%s' % (_cal_col_r_list[_i], cal_tags)] + \
                    self.honeybee_info_result['cpc_%s_call_out_%s' % (_cal_col_r_list[_i], cal_tags)]

                self.honeybee_info_result['cpc_%s_call_in_%s_ratio' % (_cal_col_r_list[_i], cal_tags)] = \
                    _cal_data['region_call_in_%s' % cal_tags].sum() / self.honeybee_info_result[_r_f.get(cal_tags)]
                self.honeybee_info_result['cpc_%s_call_out_%s_ratio' % (_cal_col_r_list[_i], cal_tags)] = \
                    _cal_data['region_call_out_%s' % cal_tags].sum() / self.honeybee_info_result[_r_f.get(cal_tags)]
                self.honeybee_info_result['cpc_%s_call_%s_ratio' % (_cal_col_r_list[_i], cal_tags)] = \
                    self.honeybee_info_result['cpc_%s_call_%s' % (_cal_col_r_list[_i], cal_tags)] / \
                    self.honeybee_info_result[_r_f.get(cal_tags)]
        else:
            _result_tep_tuple = ('cpc_%s_call_in_%s', 'cpc_%s_call_out_%s', 'cpc_%s_call_%s', 'cpc_%s_call_in_%s_ratio',
                                 'cpc_%s_call_out_%s_ratio', 'cpc_%s_call_%s_ratio')
            for _i in range(len(_cal_col_list)):
                for _t in _result_tep_tuple:
                    self.honeybee_info_result[_t % (_cal_col_r_list[_i], cal_tags)] = -999

                # self.honeybee_info_result['cpc_%s_call_in_%s' % (_cal_col_r_list[_i], cal_tags)] = -999
                # self.honeybee_info_result['cpc_%s_call_out_%s' % (_cal_col_r_list[_i], cal_tags)] = -999
                # self.honeybee_info_result['cpc_%s_call_%s' % (_cal_col_r_list[_i], cal_tags)] = -999
                # self.honeybee_info_result['cpc_%s_call_in_%s_ratio' % (_cal_col_r_list[_i], cal_tags)] = -999
                # self.honeybee_info_result['cpc_%s_call_out_%s_ratio' % (_cal_col_r_list[_i], cal_tags)] = -999
                # self.honeybee_info_result['cpc_%s_call_%s_ratio' % (_cal_col_r_list[_i], cal_tags)] = -999

    def calculate_region_info_func(self):
        # 先这样写吧 不使用嵌套多层字符串替换循环啦
        ...
        for _i in range(len(result_col)):
            ...
            # _t_col_tuple = ('cpc_%s_pct', 'cpc_%s_call_in_cnt', 'cpc_%s_call_out_cnt', )
