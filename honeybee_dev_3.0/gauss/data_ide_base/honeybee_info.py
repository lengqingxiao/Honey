...
class CalculateTagsForHoneybee(ProcessDataForHoneybee):
    ...
    def calculate_main_service_info(self):
        ...
        if not self.main_service.empty:
            ...
            try:
                ...
                data_p1m = pd.DataFrame(columns=['interact_mth', 'interact_cnt', 'total_service_cnt', 'company_type', 'company_name'])
                # data_p1m.columns = ['interact_mth', 'interact_cnt', 'total_service_cnt', 'company_type', 'company_name']
    ...
    def calculate_behavior(self):
        ...
        if not self.behavior_check.empty:
            ...
            for i in BEHAVIOR_CHECK_TUPLE:
                self.honeybee_info_result[i] = -999
...
if __name__ == '__main__':
    ...
    # result = []
    for i in ['15625867469', '18168724419', '18677827374', '15831842620', '18382035538', '18329468073']:
        r_data = QueryData.query_data_from_mongodb('127.0.0.1', 27017, 'elephant', 'honey_bee',
                                                   ['report_data', 'mobile'], 'mobile', i)
        logging.info('数据拉取完成，开始计算')
    ...
    # result_df.to_csv('D:/result_dev_0.1.csv')
# import sys
# reload(sys)
# sys.setdefaultencoding('utf8')
