...
from data_transfer.data_transform import *
...
class ProcessDataForJD(object):
    # 15755158185 13032343344 18862323143
    def __init__(self):
        # 18862323143 18841722332 13048090404
        ...
        #                                                    'addrs', 'orders', 'submitTime'], 'mobile', '18645890670')
        logging.info('数据获取完成')
        self.raw_data = QueryData.query_data_from_mongodb_func('127.0.0.1', 27017, 'elephant', 'jd',
                                                          ['mobile', 'level', 'securityLevel', 'yue', 'eCard',
                                                           'xiaojinku', 'baitiao', 'baitiaoDebt', 'xiaobai',
                                                           'addrs', 'orders', 'submitTime'], 'mobile', '18633965551')
        # self.raw_data = QueryData.query_data_from_mongodb('127.0.0.1', 27017, 'elephant', 'honey_bee',
        #                                                   ['report_data', 'mobile'], 'mobile', '18382035538')
    ...
    def process_address_data(self):
        if not self.jd_delivery_address.empty:
            _r_f = self.jd_delivery_address[0].apply(lambda x: pd.Series([i for i in x.split()]))
            # print _r_f
            _rf = _r_f[[0, 2, 4]].rename(columns={0: 'name', 2: 'loc_sec', 4: 'detail_address'})
            logging.info('load jieba model')
            _pvc_list = [ProvinceRecognize.get_address_province(_i) for _i in _rf['loc_sec'].values]
            logging.info('load finish, reshape finish')
...
k = jd.jd_order_info
print(k)
# print(type(k['title'].values[0]))
# m = k[k['order_status'] == 1]
# # print m
# n = m.groupby('time').count().reset_index()
# print n
# n1 = m.groupby('year_month_day').count().reset_index()
# n2 = n1[n1['amount_f'] == 1]
# print n2.max()
#
# result = pd.DataFrame()
...
# print result
    # m1 = pd.DataFrame(m)
# n = m1.groupby(by=['year_month_day']).agg({'amount_f': 'sum'}).reset_index()
# print n
