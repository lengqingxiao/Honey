...
class CalculateTagsForJD(ProcessDataForJD):
    ...
    def select_jd_order_data(self, pre_months):
        """
        This function use for select data by time
        ----
        param  pre_months : int; (pre months number)
        ----
        return result: selected DataFrame(Pandas DataFrame)
        usage : pass
    ...
    def calculate_jd_basic_info(self):
        _result_col = ['csm_jd_xb_score', 'csm_jd_user_bt_line', 'csm_jd_user_bt_used_amt', 'csm_jd_user_acc_scl',
                       'csm_jd_user_acc_level', 'csm_jd_user_acc_balance', 'csm_jd_user_acc_xjk',
                       'csm_jd_user_submit_time']
        _query_col = ['xiaobai', 'baitiao', 'baitiaoDebt', 'securityLevel', 'level', 'yue', 'xiaojinku', 'submitTime']
        _default_value = [-999, -999, -999, '-999', '-999', -999, -999, -999]
        _loop_tuple = zip(_result_col, _query_col, _default_value)
        for _i in _loop_tuple:
            self.jd_result_info[_i[0]] = pd.Series(self.raw_data[0].get(_i[1], _i[2]))
    ...
    def calculate_jd_region_info(self):
        ...
        if not self.jd_delivery_address_df.empty:
            ...
            self.jd_result_info['csm_jd_csg_max_ra_cnt'] = _group_df['loc_sec'].drop_duplicates().max()
            ...
            self.jd_result_info['csm_jd_csg_max_pvc_cnt'] = _pvc_cnt_df['pvc_cnt'].drop_duplicates().max()
            ...
            self.jd_result_info['csm_jd_acc_ra_cnt'] = -999
            self.jd_result_info['csm_jd_acc_csg_cnt'] = -999
            self.jd_result_info['csm_jd_acc_ra_pvc_cnt'] = -999
            self.jd_result_info['csm_jd_acc_ra_ias'] = -999
            self.jd_result_info['csm_jd_csg_max_ra_cnt'] = -999
            self.jd_result_info['csm_jd_ra_fre_name'] = '-999'
            self.jd_result_info['csm_jd_csg_max_pvc_cnt'] = -999
            self.jd_result_info['csm_jd_csg_mra_cnt'] = -999
            self.jd_result_info['csm_jd_csg_sra_cnt'] = -999
            self.jd_result_info['csm_jd_acc_if_mp_mra'] = -999
            self.jd_result_info['csm_jd_acc_if_mp_sra'] = -999
    ...
    def calculate_jd_order_basic_func(self):
        """
         This function use for calculate jd account order data basic info
         ----
         result in class result DataFrame(Pandas DataFrame)
         if no jd order data , func will use default values -999 / '-999'; data profile tags will use 0 or 1
         ----
         usage: pass

        """
        _status_tags = {'cpd': 1, 'cnc': 2}
        _cal_tags = ['cnt', 'amt']
        if not self.jd_order_info.empty:
            ...
            for _k, _v in _status_tags.items():
                self.jd_result_info['csm_jd_%s_order_cnt' % _k] = \
                    self.jd_order_info[self.jd_order_info['order_status'] == _v].shape[0]
                self.jd_result_info['csm_jd_%s_order_amt' % _k] = \
                    self.jd_order_info[self.jd_order_info['order_status'] == _v]['amount_f'].sum()
                self.jd_result_info['csm_jd_%s_order_cnt_ratio' % _k] = \
                    self.jd_result_info['csm_jd_%s_order_cnt' % _k] / self.jd_result_info['csm_jd_order_cnt']
                self.jd_result_info['csm_jd_%s_order_amt_ratio' % _k] = \
                    self.jd_result_info['csm_jd_%s_order_amt' % _k] / self.jd_result_info['csm_jd_order_amt']
                self.jd_result_info['csm_jd_%s_order_avg_amt' % _k] = \
                    self.jd_result_info['csm_jd_%s_order_amt' % _k] / self.jd_result_info['csm_jd_cpd_order_cnt']
            ...
            for _kd in _status_tags.keys():
                for _vd in _cal_tags:
                    self.jd_result_info['csm_jd_order_%s' % _vd] = -999
                    self.jd_result_info['csm_jd_%s_order_%s' % (_kd, _vd)] = -999
                    self.jd_result_info['csm_jd_%s_order_%s_ratio' % (_kd, _vd)] = -999
                self.jd_result_info['csm_jd_%s_order_avg_amt' % _kd] = -999
            self.jd_result_info['csm_jd_order_avg_amt'] = -999

    def batch_calculate_jd_order_basic_func(self):
        """
         This function use for batch calculate jd account order data basic info
         ----
         batch calculate section data; contains time tags p1m, p2m, p3m, p6m
         result in class result DataFrame(Pandas DataFrame)
         if no jd order data , func will use default values -999 / '-999'; data profile tags will use 0 or 1
         ----
         usage: pass
        ...
        if not self.jd_order_info.empty:
            ...
            for _k, _v in _cal_loop_dict.items():
                ...
                for _ks, _vs in _status_tags.items():
                    self.jd_result_info['csm_jd_%s_order_cnt_%s' % (_ks, _k)] = \
                        _v[_v['order_status'] == _vs].shape[0]
                    self.jd_result_info['csm_jd_%s_order_amt_%s' % (_ks, _k)] = \
                        _v[_v['order_status'] == _vs]['amount_f'].sum()
                    for _kt in _cal_tags:
                        self.jd_result_info['csm_jd_%s_order_%s_ratio_%s' % (_ks, _kt, _k)] = \
                            self.jd_result_info['csm_jd_%s_order_%s_%s' % (_ks, _kt, _k)] / self.jd_result_info[
                                'csm_jd_order_%s_%s' % (_kt, _k)]
                    self.jd_result_info['csm_jd_%s_order_avg_amt_%s' % (_ks, _k)] = \
                        self.jd_result_info['csm_jd_%s_order_amt_%s' % (_ks, _k)] / self.jd_result_info[
                            'csm_jd_%s_order_cnt_%s' % (_ks, _k)]
            ...
            for _k in _cal_time_tags:
                self.jd_result_info['csm_jd_order_cnt_%s' % _k] = -999
                self.jd_result_info['csm_jd_order_amt_%s' % _k] = -999
                self.jd_result_info['csm_jd_order_avg_amt_%s' % _k] = -999
                for _ks in _status_tags.keys():
                    self.jd_result_info['csm_jd_%s_order_cnt_%s' % (_ks, _k)] = -999
                    self.jd_result_info['csm_jd_%s_order_amt_%s' % (_ks, _k)] = -999
                    for _kt in _cal_tags:
                        self.jd_result_info['csm_jd_%s_order_%s_ratio_%s' % (_ks, _kt, _k)] = -999
                    self.jd_result_info['csm_jd_%s_order_avg_amt_%s' % (_ks, _k)] = -999

    def calculate_jd_order_status_func(self):
        """
         This function use for calculate jd account order data status info
         ----
         result:
         result in class result DataFrame(Pandas DataFrame)
         result DataFrame name : jd_result_info
         if not have jd order data , func will use default values -999 / '-999'; data profile tags will use 0 or 1
         ----
         usage: pass
        ...
        if not self.jd_order_info.empty:
            ...
            for _k, _v in _cal_query_tags_dict.items():
                self.jd_result_info['csm_jd_%s_order_cnt' % _k] = -999
                self.jd_result_info['csm_jd_%s_order_amt' % _k] = -999
                self.jd_result_info['csm_jd_%s_order_avg_amt' % _k] = -999
                for _i in _cal_loop_tags:
                    self.jd_result_info['csm_jd_%s_order_%s_ratio' % (_k, _i)] = -999
                for _m, _n in _cal_order_status.items():
                    self.jd_result_info['csm_jd_%s_%s_order_cnt' % (_k, _m)] = -999
                    self.jd_result_info['csm_jd_%s_%s_order_amt' % (_k, _m)] = -999
                    self.jd_result_info['csm_jd_%s_%s_order_avg_amt' % (_k, _m)] = -999
                    for _i in _cal_loop_tags:
                        self.jd_result_info['csm_jd_%s_order_%s_%s_%s_ratio' % (_k, _m, _i, _m)] = -999
                        self.jd_result_info['csm_jd_%s_order_%s_%s_total_ratio' % (_k, _m, _i)] = -999

    def batch_calculate_jd_order_status_func(self):
        """
         This function use for batch calculate jd account order data status info
         计算京东账户订单信息状态相关信息
         ----
         结果:
         计算切片后数据集，包含的时间段: 前1月，前2月，前3月，前6月
         计算结果写入类结果属性数据框
         计算结果数据框名称: jd_result_info
         如果没有京东订单相关数据，将使用默认值，数值型 -999，字符型’-999‘，
         如果不需要空数据默认值，请注释掉if not 同级的else 代码段即可。
         result:
         batch calculate section data; contains time tags p1m, p2m, p3m, p6m
         result in class result DataFrame(Pandas DataFrame)
         result DataFrame name : jd_result_info
        ...
        if not self.jd_order_info.empty:
            ...
            for _p in _cal_time_tags:
                for _k in _cal_query_tags_dict.keys():
                    self.jd_result_info['csm_jd_%s_order_cnt_%s' % (_k, _p)] = -999
                    self.jd_result_info['csm_jd_%s_order_amt_%s' % (_k, _p)] = -999
                    self.jd_result_info['csm_jd_%s_order_avg_amt_%s' % (_k, _p)] = -999
                    for _i in _cal_loop_tags:
                        self.jd_result_info['csm_jd_%s_order_%s_ratio_%s' % (_k, _i, _p)] = -999
                    for _m in _cal_order_status.keys():
                        self.jd_result_info['csm_jd_%s_%s_order_cnt_%s' % (_k, _m, _p)] = -999
                        self.jd_result_info['csm_jd_%s_%s_order_amt_%s' % (_k, _m, _p)] = -999
                        self.jd_result_info['csm_jd_%s_%s_order_avg_amt_%s' % (_k, _m, _p)] = -999
                        for _i in _cal_loop_tags:
                            self.jd_result_info['csm_jd_%s_order_%s_%s_%s_ratio_%s' % (_k, _m, _i, _m, _p)] = -999
                            self.jd_result_info['csm_jd_%s_order_%s_%s_total_ratio_%s' % (_k, _m, _i, _p)] = -999

    @classmethod
    def group_jd_order_data(cls, raw_df, col_name):
        if not raw_df.empty:
            _gp_count_data = raw_df.groupby(col_name).count().reset_index()
            _gp_sort_data = _gp_count_data.sort_values(by=['amount_f', col_name], ascending=False)
            _gp_amt = raw_df.groupby(by=[col_name]).agg({'amount_f': 'sum'}).reset_index()
            _gp_amt_sort = _gp_amt.sort_values(by=['amount_f', col_name], ascending=False)
            return _gp_count_data, _gp_sort_data, _gp_amt_sort

    def calculate_group_data_tags_func(self, gp_tuple, col_list, values_col, tags_var):

        # tags_var 可以传元祖 values_col = 'year_month'
        if not gp_tuple[0].empty:
            self.jd_result_info[col_list[0] % tags_var] = gp_tuple[0]['amount'].drop_duplicates().max()
        else:
            self.jd_result_info[col_list[0] % tags_var] = 0
        if not gp_tuple[1].empty:
            self.jd_result_info[col_list[1] % tags_var] = gp_tuple[1][values_col].values[0]
            self.jd_result_info[col_list[2] % tags_var] = self.jd_result_info[col_list[1] % tags_var].values[0][5:7]
        else:
            self.jd_result_info[col_list[1] % tags_var] = '-999'
            self.jd_result_info[col_list[2] % tags_var] = '-999'
        if not gp_tuple[2].empty:
            self.jd_result_info[col_list[3] % tags_var] = gp_tuple[2]['amount_f'].values[0]
            self.jd_result_info[col_list[4] % tags_var] = gp_tuple[2][values_col].values[0]
            self.jd_result_info[col_list[5] % tags_var] = \
                self.jd_result_info[col_list[4] % tags_var].values[0][5:7]
        else:
            self.jd_result_info[col_list[3] % tags_var] = -999
            self.jd_result_info[col_list[4] % tags_var] = '-999'
            self.jd_result_info[col_list[5] % tags_var] = '-999'
#
    def calculate_jd_order_performance_m_func(self):
        # 空参模板后续加 函数更换
        _order_status_tags = {'f': 1, 's': 2}
        _cal_status_tags_dict = {'bt': 2, 'ubt': 1, 'cod': 4}
        if not self.jd_order_info.empty:
            _smf_data = self.jd_order_info[self.jd_order_info['order_status'] == 1]
            for _ps, _qs in _order_status_tags.items():
                _ssd_data = self.jd_order_info[self.jd_order_info['order_status'] == _qs]
                if not _ssd_data.empty:
                    _gpd_tuple = self.group_jd_order_data(_ssd_data, 'year_month')
                    if not _gpd_tuple[0].empty:
                        self.jd_result_info['csm_jd_sm%s_order_max_cnt' % _ps] = \
                            _gpd_tuple[0]['amount'].drop_duplicates().max()
                    else:
                        self.jd_result_info['csm_jd_sm%s_order_max_cnt' % _ps] = 0
                    if not _gpd_tuple[1].empty:
                        self.jd_result_info['csm_jd_sm%s_order_max_cnt_rt' % _ps] = \
                            _gpd_tuple[1]['year_month'].values[0]
                        self.jd_result_info['csm_jd_sm%s_order_max_cnt_mt' % _ps] = \
                            self.jd_result_info['csm_jd_sm%s_order_max_cnt_rt' % _ps].values[0][5:7]
                    else:
                        self.jd_result_info['csm_jd_sm%s_order_max_cnt_rt' % _ps] = '-999'
                        self.jd_result_info['csm_jd_sm%s_order_max_cnt_mt' % _ps] = '-999'
                    if not _gpd_tuple[2].empty:
                        self.jd_result_info['csm_jd_sm%s_order_max_amt' % _ps] = _gpd_tuple[2]['amount_f'].values[0]
                        self.jd_result_info['csm_jd_sm%s_order_max_amt_rt' % _ps] = _gpd_tuple[2]['year_month'].values[0]
                        self.jd_result_info['csm_jd_sm%s_order_max_amt_mt' % _ps] = \
                            self.jd_result_info['csm_jd_sm%s_order_max_amt_rt' % _ps].values[0][5:7]
                    else:
                        self.jd_result_info['csm_jd_sm%s_order_max_amt' % _ps] = -999
                        self.jd_result_info['csm_jd_sm%s_order_max_amt_rt' % _ps] = '-999'
                        self.jd_result_info['csm_jd_sm%s_order_max_amt_mt' % _ps] = '-999'
#####
            for _k, _v in _cal_status_tags_dict.items():
                _smf_data_sec = _smf_data[_smf_data['pay_channel'] == _v]
                if not _smf_data_sec.empty:
                    _sec_tuple = self.group_jd_order_data(_smf_data_sec, 'year_month')
    ...
    def calculate_jd_order_type_info(self):
        ...
        if not self.jd_order_info.empty:
            for _k, _v in _cal_dict.items():
                ...
                if _cal_data.shape[0] > 1:
                    _time_diff_df = pd.DataFrame()
                    _time_diff_df['time_a'] = _cal_data.reset_index()['year_month_day'].drop_duplicates().sort_values(ascending=False)
                    # _a = list(_time_diff_df['time_a'])
                    # print _a[1:]
                    _time_diff_df['time_b'] = pd.Series(list(_time_diff_df['time_a'])[1:])
                    # print _time_diff_df[: -1]
                    # _diff_df = _time_diff_df.drop(_time_diff_df.shape[0])

                    _diff_df = _time_diff_df.dropna()
                    if not _diff_df.empty:
                    # print _diff_df
                    #     _diff_df_c = _diff_df.loc[:, ['time_a', 'time_b']]
                        # _diff_df['time_a_n'] = _diff_df_c['time_a'].apply(self.calculate_time_func_n)
                        _diff_df['time_a_n'] = _diff_df['time_a'].apply(self.calculate_time_func_n)
                        _diff_df['time_b_n'] = _diff_df['time_b'].apply(self.calculate_time_func_n)
                        # _diff_df_cc = _diff_df.loc[:, ['time_a_n', 'time_b_n']]
                        _diff_df['diff_days'] = (_diff_df['time_a_n'] - _diff_df['time_b_n']).apply(self.calculate_diff_days)
                        self.jd_result_info['csm_jd_buy_%s_max_tsp' % _k] = _diff_df['diff_days'].drop_duplicates().max()
                        self.jd_result_info['csm_jd_buy_%s_min_tsp' % _k] = _diff_df['diff_days'].drop_duplicates().min()
                        self.jd_result_info['csm_jd_buy_%s_avg_tsp' % _k] = _diff_df['diff_days'].sum()/_diff_df.shape[0]
                    else:
                        self.jd_result_info['csm_jd_buy_%s_max_tsp' % _k] = -999
                        self.jd_result_info['csm_jd_buy_%s_min_tsp' % _k] = -999
                        self.jd_result_info['csm_jd_buy_%s_avg_tsp' % _k] = -999
                    # print _diff_df
                    # _time_diff_df['time_a']
                    # print _time_diff_df
    ...
    def calculate_jd_order_trade_other_info(self):
        ...
        _cal_time_tags = ['p1m', 'p2m', 'p3m', 'p6m']
        _cal_loop_dict = dict(zip(_cal_time_tags, _cal_data_list))
        _wait_pay_data = self.jd_order_info[self.jd_order_info['order_status'] == 3]
        self.jd_result_info['csm_jd_order_wait_pay_cnt_p1m'] = _wait_pay_data.shape[0]
        self.jd_result_info['csm_jd_order_wait_pay_amt_p1m'] = _wait_pay_data['amount_f'].sum()
        if not self.jd_order_info.empty:
            _smf_data = self.jd_order_info[self.jd_order_info['order_status'] == 1]
            ...
            _jd_shop_f1 = _smf_data[_smf_data['if_order_use_jd_shop'] == 1]
            self.jd_result_info['csm_jd_order_use_jsp_cnt'] = _jd_shop_f1.shape[0]
            self.jd_result_info['csm_jd_order_use_jsp_amt'] = _jd_shop_f1['amount_f'].sum()
            self.jd_result_info['csm_jd_order_use_jsp_avg_amt'] = \
                self.jd_result_info['csm_jd_order_use_jsp_amt'] / \
                self.jd_result_info['csm_jd_order_use_jsp_cnt']
            _jd_shop_s1 = self.jd_order_info[self.jd_order_info['if_order_use_jd_send'] == 1]
            self.jd_result_info['csm_jd_order_use_jsd_cnt'] = _jd_shop_s1.shape[0]
            self.jd_result_info['csm_jd_order_use_jsd_cnt_ratio'] = \
                self.jd_result_info['csm_jd_order_use_jsd_cnt'] / self.jd_result_info['csm_jd_cpd_order_cnt']
            for _i in _cal_category:
                ...
                # _count_cal_category_fre = collections.Counter(_cal_category_fre)
                _cal_category_fre_dict = dict(collections.Counter(_cal_category_fre))
                if len(_cal_category_fre_dict) > 0:
        ...
        for _k, _v in _cal_loop_dict.items():
            ...
            self.jd_result_info['csm_jd_order_max_cnt_stp_%s' % _k] = \
                _pmf_count_data['amount_f'].drop_duplicates().max()
            ...
            _jd_shop_f = _pmf_data[_pmf_data['if_order_use_jd_shop'] == 1]
            self.jd_result_info['csm_jd_order_use_jsp_cnt_%s' % _k] = _jd_shop_f.shape[0]
            self.jd_result_info['csm_jd_order_use_jsp_amt_%s' % _k] = _jd_shop_f['amount_f'].sum()
            self.jd_result_info['csm_jd_order_use_jsp_avg_amt_%s' % _k] = \
                self.jd_result_info['csm_jd_order_use_jsp_amt_%s' % _k] / \
                self.jd_result_info['csm_jd_order_use_jsp_cnt_%s' % _k]
            _jd_shop_s = _v[_v['if_order_use_jd_send'] == 1]
            self.jd_result_info['csm_jd_order_use_jsd_cnt_%s' % _k] = _jd_shop_s.shape[0]
            self.jd_result_info['csm_jd_order_use_jsd_cnt_ratio_%s' % _k] = \
                self.jd_result_info['csm_jd_order_use_jsd_cnt_%s' % _k] / \
...
if __name__ == '__main__':
    ...
    # print da[
    #     ['csm_jd_user_trade_time_top_1_p6m', 'csm_jd_user_trade_time_top_2_p6m', 'csm_jd_user_trade_time_top_3_p6m','csm_jd_buy_trv_max_tsp']]
    # for i in da.columns:
    #     print da[i]
